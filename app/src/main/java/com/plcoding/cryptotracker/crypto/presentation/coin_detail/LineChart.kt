...
fun LineChart(
    ...
    LaunchedEffect(xLabelWidth) {
        onXLabelWidthChange.invoke(xLabelWidth)
    }
    ...
    Canvas(
            ...
            .pointerInput(dataPoints, xLabelWidth) {
                detectHorizontalDragGestures { change, _ ->
                    ...
                    if (isShowingDataPoints) {
                        onSelectedDataPoint.invoke(dataPoints[newSelectedDataPointIndex])
                    }
                }
            },
    ) {
        ...
        val xLabelLineHeight = maxXLabelHeight / maxXLabelLineCount // ukuran height label dari setiap line
...
//        )
            ...
            if (showHelperLines) {
                drawLine(
                    ...
                    strokeWidth = if (selectedDataPointIndex == index) {
                        style.helperLinesThicknessPx * 5f
        ...
        val heightRequiredLabels = xLabelLineHeight *
                (labelCountExcludingLastLabel + 1)
        val remainingHeightForLabels = labelViewPortHeightPx - heightRequiredLabels
...
fun LineCharPreview() {
    CryptoTrackerTheme {
        val coinHistoryRandomzied = remember {
            (1..200).map {
                CoinPrice(
